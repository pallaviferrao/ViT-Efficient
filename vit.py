...
class PatchEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.projection = nn.Conv2d(self.num_channels, self.hidden_size, kernel_size=self.patch_size,
...
class KVCacheAttention(nn.Module):
    ...
    def forward(self, x, output_attentions=False, past_key_value = None,):
...
class MixtureOfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attention_head_size = self.hidden_size // self.num_attention_heads
        self.shared_num = 2
        self.total_routed_attention_heads = 4
        self.routed_head = 2
...
class ViTForClassfication(nn.Module):
    ...
    def forward(self, x, set_training, output_attentions=False):
        # Calculate the embedding output
        embedding_output = self.embedding(x)
        # Calculate the encoder's output
        encoder_output, all_attentions = self.encoder(embedding_output, is_training =  set_training,  output_attentions=output_attentions)
