...
class LongformerSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.query = nn.Linear(self.hidden_size, self.embed_dim)
        self.key = nn.Linear(self.hidden_size, self.embed_dim)
        self.value = nn.Linear(self.hidden_size, self.embed_dim)

        self.query_global = nn.Linear(self.hidden_size, self.embed_dim)
        self.key_global = nn.Linear(self.hidden_size, self.embed_dim)
        self.value_global = nn.Linear(self.hidden_size, self.embed_dim)
    ...
    def forward(
        ...
        seq_len, bsz, embed_dim_actual = hidden_states.size()
        assert embed_dim_actual == embed_dim

        # Compute padding needed
        pad_len = (8 - (seq_len % 8)) % 8
        padded_seq_len = seq_len + pad_len  # Ensure it's a multiple of `pad_multiple`

        # Pad hidden states along sequence dimension (dim=0)
        if pad_len > 0:
            hidden_states = F.pad(hidden_states, (0, 0, 0, 0, 0, pad_len), value=0)
        ...
        # Reshape Q, K
        q = q.view(padded_seq_len, bsz, self.num_heads, self.head_dim).transpose(0, 1)
        k = k.view(padded_seq_len, bsz, self.num_heads, self.head_dim).transpose(0, 1)
        ...
        if remove_from_windowed_attention_mask is not None:
            ...
            attn_weights += d_mask
        assert list(attn_weights.size())[:3] == [bsz, padded_seq_len, self.num_heads]
        ...
        assert list(attn.size()) == [bsz, padded_seq_len, self.num_heads, self.head_dim]
        attn = attn.transpose(0, 1).reshape(padded_seq_len, bsz, embed_dim).contiguous()
