...
class MixtureOfAttentionHeterogenousToppSparseShare(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.strided_routed_attention_heads = int(self.total_routed_attention_heads / 2)
        self.both_routed_attention_heads = int(self.total_routed_attention_heads / 2)
    ...
    def forward(self,x, output_attentions=False, is_training= False):
        ...
        MixtureOfAttentionHeterogenousToppSparseShare.LOAD_BALANCING_LOSSES.append(l_aux)
        ...
        attention_probs = torch.cat(
            (
                ...
                attention_probs[:, self.shared_num: self.shared_num+ self.strided_routed_attention_heads, :, :].masked_fill(self.attn_mask_strided == 0,
                                                                                           0),
                attention_probs[:, self.shared_num + self.strided_routed_attention_heads:, :, :].masked_fill(self.attn_mask_both == 0, 0)
            ), dim=1
        )
