from Layers.Attention.Longformer.diagonal_tvm import _get_invalid_locations_mask
import torch
...
#     It handles odd sequence lengths by padding them appropriately.'''
#
#     bsz, seqlen, num_heads, head_dim = q.size()
#     breakpoint()
#     # Check if padding is needed
#     pad_len = (w * 2) - (seqlen % (w * 2)) if seqlen % (w * 2) != 0 else 0
#     padded_seqlen = seqlen + pad_len  # Ensure it is divisible by 2w
#
#     # Pad tensors if needed
#     if pad_len > 0:
#         q = F.pad(q, (0, 0, 0, pad_len), value=0)  # Pad along sequence dim
#         k = F.pad(k, (0, 0, 0, pad_len), value=0)
#
#     assert padded_seqlen % (w * 2) == 0
#     assert q.size() == k.size()
#
#     chunks_count = padded_seqlen // w - 1
#
#     # Reshape for chunking
#     q = q.transpose(1, 2).reshape(bsz * num_heads, padded_seqlen, -1)
#     k = k.transpose(1, 2).reshape(bsz * num_heads, padded_seqlen, -1)
#
#     chunk_q = _chunk(q, w)
#     chunk_k = _chunk(k, w)
#
...
#
#     # Allocate space for combined attention
#     diagonal_attn = diagonal_chunk_attn.new_empty((bsz * num_heads, chunks_count + 1, w, w * 2 + 1))
#
#     # Copy parts from diagonal_chunk_attn
#     diagonal_attn[:, :-1, :, w:] = diagonal_chunk_attn[:, :, :w, :w + 1]
#     diagonal_attn[:, -1, :, w:] = diagonal_chunk_attn[:, -1, w:, :w + 1]
#     diagonal_attn[:, 1:, :, :w] = diagonal_chunk_attn[:, :, -(w + 1):-1, w + 1:]
#     diagonal_attn[:, 0, 1:w, 1:w] = diagonal_chunk_attn[:, 0, :w - 1, 1 - w:]
#
#     # Reshape back to (bsz, num_heads, seqlen, 2 * w + 1)
#     diagonal_attn = diagonal_attn.view(bsz, num_heads, padded_seqlen, 2 * w + 1).transpose(2, 1)
#
#     # Remove extra padding from the attention matrix
#     if pad_len > 0:
#         diagonal_attn = diagonal_attn[:, :, :seqlen, :]  # Remove padded tokens
#
